...
coins <- c("BTC-USD", "BNB-USD", "ETH-USD", "XRP-USD", "LTC-USD", "DOGE-USD", "SPY", "^TNX", "^VIX", "DXY")
...
dxy_ret = as.numeric(na.omit(diff(log(`DXY`[,6]))))

df <- data.frame(cbind(btc_ret,bnb_ret,eth_ret,xrp_ret,ltc_ret,doge_ret,vix_ret,spy_ret,tnx_ret,dxy_ret))
...
ctest1t <- ca.jo(df, type = "trace", ecdet = "const", K = 2)
summary(ctest1t)
#pass

#Johansen Testing (MaxEigen)
ctest1e <- ca.jo(df, type = "eigen", ecdet = "const", K = 2)
summary(ctest1e)
#pass
